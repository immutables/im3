...
import io.immutables.lang.typeold2.Unify.Solution;
import io.immutables.meta.Null;
import java.util.*;
import java.util.function.Consumer;
import java.util.function.Function;
import static io.immutables.lang.typeold2.Types.traverse;
...
public class Solver {
	...
	private @Null Resolution resolution;
	...
	static Proposition conforms(Type expected, Type actual) {
		return new Proposition() {
			@Override public void collectVariables(Consumer<Type.Variable> consumer) {
				traverse(expected, t -> {if (t instanceof Type.Variable v) consumer.accept(v);});
				traverse(actual, t -> {if (t instanceof Type.Variable v) consumer.accept(v);});
			}
	...
	public static void typecheck(Type expected, Type actual) {
		var solver = solver(conforms(expected, actual));
		solver.solve();
		var r = requireNonNullElse(solver.resolution, Resolution.Impossible);
