...
*/
  ...
  static final class ResultSetIn implements In {
		...
		public void beginStruct(NameIndex names) throws IOException {
			...
			if (this.names != names || names.isDynamic()) {
				this.names = names;
				for (int i = 0; i < columnCount; i++) {
					indexes[i] = names.nameToIndex(names[i]);
				}
			}
			advanceColumn();
		}
	...
	static class StatementParameterOut implements Out {
		...
		private NameIndex spreadingIndex;

		StatementParameterOut(NameIndex parameterIndex) {
		...
		private NameIndex fields() {
			return spreading == SpreadState.Doing ? spreadingIndex : parameterIndex;
		}

		@Override
		public void putField( int field) throws IOException {
    ...
    ColumnExtractor(Codec<Object> codec, SqlAccessor.Column column) {
      ...
      this.column = column;
		...
		public Object decode(In in) throws IOException {
			...
			NameIndex fields = Codec.arbitraryFields();
